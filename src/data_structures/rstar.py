...
class BoundingBox:
    ...
    def asymmetry(self, other: 'BoundingBox', dim: int):
        ...
        return 2*(self.center_along(dim) - other.center_along(dim))/self.width(dim)
...
class RSNode:
    def __init__(self, parent=None, lower=5, upper=50, shape=0.5):
        # max is max # of values per node before splitting
        # min is min # of values in a non-root node
        self.__lower = lower
        self.__upper = upper
        # controls gaussian parameter shape
        self.__shape = shape
    ...
    def choose_split(self):
        ...
        if self.is_leaf():
            ...
            for dim in range(len(self.bounds.tops.shape)):
                ...
                # computing wg stuff
                for i in range(self.__lower, self.__upper-self.__lower+1):
                    sc_i: List[NDArray[BoundingBox]] = np.split(np.stack([top_sort_bbs, bot_sort_bbs], axis=0), [i], axis=1)
                ...
                asym = np.append(asym, self.bounds.asymmetry(self.o_box, dim))
            # computing wf stuff
            ...
            y_s = 1/(1 - y1)
            # dependent variable
            xi = 2*np.arange(self.__lower, self.upper-self.__lower+1)/(self.__upper+1) - 1

            wf: NDArray[float] = y_s *(np.exp(-1*(((xi-mean)/sigma)**2)) - y1)
